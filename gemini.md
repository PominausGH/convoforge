...
behavioral intelligence platform for communication coaching, built as a **World App Mini App**.

ConvoForge runs inside World App using the MiniKit SDK. World ID provides Sybil-resistant
identity (one human = one account, zero duplicates). Payments use Stripe.
This is NOT a Carnegie app — it is a full-stack **"communication OS"**.
...
Orb-verified users unlock the **Sincerity Detector** — higher trust = deeper psychological
coaching (appropriate: Orb = provably unique human, not anonymous device).
...
**No Supabase. No extra auth layer.** World ID is the identity system. FastAPI is the API
layer. postgres-main is the database. R2 handles any file storage.
...
- Nudges styled as World App native toast notifications via MiniKit
...
**Sessions 21–30: Authentic Leader Track** *(Orb-verified only)*
Deep Carnegie principles applied to leadership, negotiation, and high-stakes conversations

### 4. Sincerity Detector (Orb tier gate)
